...
// Code source: https://habr.com/ru/articles/459004/
public class Crypto {
...
public static byte[] GOST_Kuz_Encript(byte[] blk)
{
    int i;
...
public static byte[] GOST_Kuz_Decript(byte[] blk)
{
    int i;
    byte[] out_blk = new byte[BLOCK_SIZE];
    out_blk = blk;
...
static byte[] blk = HexFormat.of().parseHex("8899aabbccddeeff0077665544332211");
    ...
    public static void main(String[] args) 
    {
        ...
        System.out.println(HexFormat.of().formatHex(encriptBlok));
        long end = System.nanoTime();
        long durationNs = end - start;

        System.out.println("Время: " + (durationNs / 1_000.0) + " микросекунд");

        byte[] decriptBlok = GOST_Kuz_Decript(encriptBlok);
        System.out.println(HexFormat.of().formatHex(decriptBlok));
    }
}
